...
public class DBDAO implements
        CustomerDAO, ManagerDAO, StationDAO, TicketPointDAO, TrainDAO, TrainOrderDAO, TrainScheduleDAO
{
    @SuppressWarnings("WeakerAccess")
    public ResultSet query(String sql){
        return DBManager.getInstance().execute(sql).query();
    }

    @SuppressWarnings("WeakerAccess")
    public int[] update(String sql){
        return DBManager.getInstance().execute(sql).update();
    }

    @SuppressWarnings("WeakerAccess")
    public ResultSet query(String sql, Object ... params){
        if(params == null || params.length == 0) return query(sql);
        return DBManager.getInstance().prepare(sql, params).query();
    }

    @SuppressWarnings("WeakerAccess")
    public int[] update(String sql, Object ... params){
        if(params == null || params.length == 0) return update(sql);
        return DBManager.getInstance().prepare(sql, params).update();
    }
    ...
    public void insertManager(Manager manager) {
        update("insert into TRAIN.\"manager\" " +
    ...
    public void deleteManager(Manager manager) {
        update("delete from TRAIN.\"manager\" where \"manager_id\" = ?", manager.getManagerId());
    }

    @Override
    public void updateManager(Manager manager) {
        update("update TRAIN.\"manager\" set \"point_id\" = ?, \"username\" = ?, \"password\" = ?, \"name\" = ?,"
                + " \"sex\" = ?, \"manager_type\" = ?" +
                " where \"manager_id\" = ?",
    ...
    public Manager getManagerById(String manager_id) {
        return new Loader<>(this::getManagerFromResult)
                .loadOne("select * from TRAIN.\"manager\" where \"manager_id\" = ?", manager_id);
    }

    @Override
    public List<Manager> getAllManagers() {
        return new Loader<>(this::getManagerFromResult)
                .load("select * from TRAIN.\"manager\"");
    }
    ...
    public List<Manager> searchManagers(String key) {
        key = '%' + key + '%';
        return new Loader<>(this::getManagerFromResult)
                .load("select * from TRAIN.\"manager\" where \"manager_id\" = ? or \"name\" like ? or \"username\" like ?" +
                                " or \"point_id\" = ?",
                        key, key, key, key);
    }
    ...
    public Manager getManagerByUsername(String username) {
        ...
        return new Loader<>(this::getManagerFromResult)
                .loadOne("select * from TRAIN.\"manager\" where \"username\" = ?", username);
    }
    ...
    public void insertCustomer(Customer customer) {
        update("insert into TRAIN.\"customer\" " +
    ...
    public void deleteCustomer(Customer customer) {
        update("delete from TRAIN.\"customer\" where \"id_num\" = ?", customer.getIdNum());
    }

    @Override
    public void updateCustomer(Customer customer) {
        update("update TRAIN.\"customer\" set \"name\" = ?, \"sex\" = ?, \"tel\" = ?, \"customer_type\" = ?" +
                        " where \"id_num\" = ?",
    ...
    public Customer getCustomerByIdNum(String id_num) {
        return new Loader<>(this::getCustomerFromResult)
                .loadOne("select * from TRAIN.\"customer\" where \"id_num\" = ?", id_num);
    }
    ...
    public List<Customer> searchCustomers(String key) {
        key = '%' + key + '%';
        return new Loader<>(this::getCustomerFromResult)
                .load("select * from TRAIN.\"customer\" where \"id_num\" like ? or \"name\" like ? or \"tel\" = ?",
                        key, key, key);
    }
    ...
    public List<Customer> getAllCustomers() {
        return new Loader<>(this::getCustomerFromResult)
                .load("select * from TRAIN.\"customer\"");
    }
    ...
    public void insertStation(Station station) {
        update("insert into TRAIN.\"station\" " +
                        "(\"station_name\") values " +
                        "(?)",
    ...
    public void deleteStation(int station_id) {
        update("delete from TRAIN.\"station\" where \"station_id\" = ?", station_id);
    }

    @Override
    public void updateStation(Station station) {
        update("update TRAIN.\"station\" set \"station_name\" = ?" +
                        " where \"station_id\" = ?",
    ...
    private Station getStationById(int id) {
        return new Loader<>(this::getStationFromResult)
                .loadOne("select * from TRAIN.\"station\" where \"station_id\" = " + id);
    }

    @Override
    public List<Station> getAllStations() {
        return new Loader<>(this::getStationFromResult)
                .load("select * from TRAIN.\"station\"");
    }

    @Override
    public void insertTicketPoint(TicketPoint ticketPoint) {
        update("insert into TRAIN.\"ticket_point\" " +
    ...
    public void deleteTicketPoint(int point_id) {
        update("delete from TRAIN.\"ticket_point\" where \"point_id\" = ?", point_id);
    }

    @Override
    public void updateTicketPoint(TicketPoint ticketPoint) {
        update("update TRAIN.\"ticket_point\" set \"username\" = ?, \"address\" = ?, \"open_time\" = ?" +
                        " where \"point_id\" = ?",
    ...
    public TicketPoint getTicketPointById(int point_id) {
        return new Loader<>(this::getTicketPointFromResult)
                .loadOne("select * from TRAIN.\"ticket_point\" where \"point_id\" = ?", point_id);
    }
    ...
    public List<TicketPoint> getAllTicketPoints() {
        return new Loader<>(this::getTicketPointFromResult)
                .load("select * from TRAIN.\"ticket_point\"");
    }
    ...
    public List<TicketPoint> searchTicketPoints(String key) {
        key = '%' + key + '%';
        return new Loader<>(this::getTicketPointFromResult)
                .load("select * from TRAIN.\"ticket_point\" where \"point_id\" = ? or \"username\" like ? or \"address\" like ?",
                        key, key, key);
    }

    @Override
    public void insertTrain(Train train) {
        update("insert into TRAIN.\"train\" " +
    ...
    public void deleteTrain(String train_id) {
        update("delete from TRAIN.\"train\" where \"train_id\" = ?", train_id);
    }

    @Override
    public void updateTrain(Train train) {
        update("update TRAIN.\"train\" set \"train_type\" = ?, \"train_passby\" = ?" +
                        " where \"train_id\" = ?",
                train.getTrainType(), train.getTrainPassbyString(),
                train.getTrainId());
        if(train.getTrainPassby() != null){
            for(Passby passby : train.getTrainPassby()){
                update("insert into TRAIN.\"passby\" (\"train_id\", \"station_order\", \"depart_station\", \"arrive_station\", "
                        + "\"distance\", \"stay_time\") " +
                        "values( ?, ?, ?, ?, ?, ? )",
                        passby.getTrain().getTrainId(), passby.getStationOrder(),
                        passby.getDepartStation().getStationId(), passby.getArriveStation().getStationId(),
        ...
        if(train.getSeats() != null){
            for(SeatGroup sg : train.getSeats()){
                for(int i = 1; i <= sg.getSeatNum(); i++){
                    update("insert into TRAIN.\"seat\" (\"train_id\", \"carriage_num\", \"seat_num\", \"seat_type\") " +
                                    "values(?, ?, ?, ?)",
    ...
    public Train getTrainById(String train_id) {
        return new Loader<>(this::getTrainFromResult)
                .loadOne("select * from TRAIN.\"train\" where \"train_id\" = ?", train_id);
    }
    ...
    private Train getTrainFromResult(ResultSet res) throws SQLException {
        ...
        ResultSet pres = query("select * from TRAIN.\"passby\" where \"train_id\" = ?", train.getTrainId());
        while(pres.next()){
            Passby p = new Passby();
            p.setTrain(train);
        ...
        pres = query("select \"train_id\", \"carriage_num\", count(\"seat_num\") as cnt, \"seat_type\" "
                + "from TRAIN.\"seat\" where \"train_id\" = ?" +
    ...
    public List<Train> searchTrains(String key) {
        ...
        return new Loader<>(this::getTrainFromResult)
                .load("select * from TRAIN.\"train\" where \"train_id\" = ? or \"train_type\" = ? or \"train_passby\" like ?",
                        key, key, searchKey);
    }
    ...
    public Seat getSeat(String train_id, int carriageNum, int seatNum) {
        return new Loader<>(this::getSeatFromResult)
                .loadOne("select * from TRAIN.\"seat\" where \"train_id\" = ? and \"carriage_num\" = ? and \"seat_num\" = ?",
                        train_id, carriageNum, seatNum);
    }

    public Seat getSeatById(int id) {
        return new Loader<>(this::getSeatFromResult)
                .loadOne("select * from TRAIN.\"seat\" where \"seat_id\" = ?",
                        id);
    }
    ...
    public int insertTrainOrder(TrainOrder order) {
        TrainOrder ordered = new Loader<>(this::getOrderFromResult)
                .loadOne("select * from TRAIN.\"ticket_order\"" +
                                " where \"sche_id\" = ? and \"id_num\" = ? and (\"order_state\" = 1 or \"order_state\" = 3)",
                        order.getTrainSchedule().getScheId(), order.getBuyer().getIdNum());
        if (ordered != null) {
            return 0;
        }
        update("insert into TRAIN.\"ticket_order\" " +
                        "(\"point_id\", \"id_num\", \"sche_id\", \"seat_id\", \"train_id\", \"depart_station_order\","
                + " \"arrive_station_order\", \"depart_station\", \"arrive_station\", \"is_stu_ticket\", "
                + "\"money\", \"vorder_state\") values " +
                        "(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
    ...
    public void deleteTrainOrder(int order_id) {
        update("delete from TRAIN.\"ticket_order\" where \"order_id\" = ?", order_id);
    }

    @Override
    public void updateTrainOrder(TrainOrder order) {
        update("update TRAIN.\"ticket_order\" set \"point_id\" = ?, \"id_num\" = ?, \"sche_id\" = ?, \"seat_id\" = ?, \"train_id\" = ?," +
    ...
    public List<TrainOrder> getTrainOrderList(int limit, int skip) {
        return new Loader<>(this::getOrderFromResult)
                .load("select \"ticket_order\".*, \"customer\".\"name\" from TRAIN.\"ticket_order\", TRAIN.\"customer\"" +
    ...
    public List<TrainOrder> searchTrainOrders(String key) {
        String likeKey = '%' + key + '%';
        return new Loader<>(this::getOrderFromResult)
                .load("select o.*, c.\"name\" from TRAIN.\"ticket_order\" as o, TRAIN.\"customer\" as c" +
    ...
    public void insertTrainSchedule(TrainSchedule schedule) {
        update("insert into TRAIN.\"train_schedule\" " +
    ...
    public void deleteTrainSchedule(int sche_id) {
        update("delete from TRAIN.\"train_schedule\" where \"sche_id\" = ?", sche_id);
    }

    @Override
    public void updateTrainSchedule(TrainSchedule schedule) {
        update("update TRAIN.\"train_schedule\" set \"depart_time\" = ?, \"presell_time\" = ?, \"speed\" = ?, \"train_id\" = ?" +
                        " where \"sche_id\" = ?",
    ...
    public List<TrainSchedule> getTrainScheduleList(int limit, int skip) {
        return new Loader<>(this::getScheduleFromResult)
                .load("select * from TRAIN.\"train_schedule\"");
    }

    public TrainSchedule getTrainScheduleById(int id) {
        return new Loader<>(this::getScheduleFromResult)
                .loadOne("select * from TRAIN.\"train_schedule\" where \"sche_id\" = ?", id);
    }

    public List<TrainSchedule> searchTrainSchedule(String key) {
        return new Loader<>(this::getScheduleFromResult)
                .load("select * from TRAIN.\"train_schedule\" where \"sche_id\" = ? or \"depart_time\" = ? or"
                        + " \"presell_time\" = ? or \"train_id\" = ?",
                        key, key, key, key);
    }
    ...
    public List<TrainSchedule> searchTrainSchedule(String depart_station, String arrive_station, Date depart_date, boolean isStudent) {
        ...
        return new Loader<>(this::fillScheduleWithTrain)
                .load("select * from TRAIN.\"train_schedule\" as s" +
    ...
    public List<SeatAggregate> getSeatAggregateInSchedule(TrainSchedule schedule, int departStationOrder, int arriveStationOrder) {
        return new Loader<>(this::getSeatAggregateFromResult)
                .load("select s.\"seat_type\", count(s.\"seat_id\") as seat_cnt from TRAIN.\"seat\" as s" +
                                " left outer join \"ticket_order\" as o on  s.\"seat_id\" = o.\"seat_id\"" +
                                " where s.\"train_id\" = ? and (\"sche_id\" is null or "
                        + "(\"sche_id\" = ? and (\"vorder_state\" is null or \"order_state\" = ? or \"order_state\" = ?" +
    ...
    public List<Seat> getFreeSeats(TrainSchedule schedule) {
        return new Loader<>(this::getSeatFromResult)
                .load("select * from TRAIN.\"seat\"" +
                                " left outer join \"ticket_order\" as o on \"seat\".\"seat_id\" = o.\"seat_id\"" +
                                " where \"seat\".\"train_id\" = ? and (o.\"sche_id\" is null or "
                        + "(o.\"sche_id\" = ? and (o.\"order_state\" = 2 or o.\"order_state\" = 4)))",
                        schedule.getTrain().getTrainId(), schedule.getScheId());
    }
}